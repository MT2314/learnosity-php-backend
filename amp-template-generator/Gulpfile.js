 ...
 */
function _sass() {
    return gulp.src('node_modules/@publishing-platform/component-library/dist/templates/scss/styles.scss')
        .pipe(sass({
...
buildCssDebug.description = 'Compiles SASS with sourcemaps, for local development';
...
var buildCssBrightcove = function() {
    return gulp.src('src/scss/brightcove.scss')
        .pipe(sourcemaps.init())
        .pipe(sourcemaps.identityMap())
        .pipe(sass().on('error', sass.logError))
        .pipe(sourcemaps.write('.'))
...
buildCssBrightcove.description = 'Compiles Brightcove SASS with sourcemaps, for local development';
...
createViewsFolder.description = 'generates views folder from node_modules and src/views, for use with transcompiler';
...
function _copyFiles() {
    return gulp.src(['views/**/*', 'package*.json', '.eleventy.js'], { base: './' })
        .pipe(gulp.dest('build/nodejs'));
}

function _copyStyles() {
    return gulp.src('.tmp/styles/*.css')
        .pipe(gulp.dest('build/nodejs/styles'));
}

function _copyGulpfile() {
    return gulp.src('Gulpfile.lambda.js')
...
function _installBuild() {
    return new Promise(function(resolve, reject) {
        var p = cp.spawn('npm', ['install', '--production'], { cwd: 'build/nodejs'});
        p.stdout.on('data', (data) => { 
            log(`${data}`); 
        });
        p.stderr.on('data', (data) => { 
...
function _zipBuild() {
    return new Promise(function(resolve, reject) {
        var p = cp.spawn('zip', ['luke-ilc-amp-layer.zip', '-r', './nodejs', '--symlinks'], { cwd: 'build'});
        p.stdout.on('data', (data) => { 
            log(`${data}`); 
        });
        p.stderr.on('data', (data) => { 
...
var build = gulp.series(
    _deleteBuildFolder,
    buildCss,
    gulp.parallel(_copyFiles, _copyStyles, _copyGulpfile),
    _installBuild,
    _zipBuild
);
