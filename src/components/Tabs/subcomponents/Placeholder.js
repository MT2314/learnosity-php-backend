...
const Placeholder = ({ isOver, getItem }) => {
  return (
    <>
      {isOver ? (
        <PlaceholderHover isOver={acceptListComp(getItem)}>
          {acceptListComp(getItem) ? (
            <Title>Add a component here!</Title>
          ) : (
            <Title
              isOver={true}
            >{`Error: ${getItem?.componentName} not complatible!`}</Title>
          )}
          <Paragraph>
            Drag and drop a component from the left panel or use your keyboard
